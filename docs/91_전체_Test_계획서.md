...
| [P1-CT-001](02_Phase1_콘텐츠_자산_빌드파이프라인_상세설계서.md#p1-ct-001) | CT | FUNC-P1-001 | WPN-0001 물리19 레벨1; 같은요청2 회 | 원문 ID 와 수치가 content_template 에 일치; 동일 commandId/payload 반복은 효과1 회, 같은 ID/다른 payload 는 IdempotencyKeyReuse | NOT_RUN |
| [P1-IT-001](02_Phase1_콘텐츠_자산_빌드파이프라인_상세설계서.md#p1-it-001) | IT | FUNC-P1-001 | WPN-0001 물리19 레벨1; 모듈 adapter 를실제 구현으로교체 | 원문 ID 와 수치가 content_template 에 일치; 앱/헤드리스 entry 가 동일핵심 use case 를호출하고 새세션으로재조회시동일결과 | NOT_RUN |
| [P1-UT-002](02_Phase1_콘텐츠_자산_빌드파이프라인_상세설계서.md#p1-ut-002) | UT | FUNC-P1-002 | 두 번 동일 소스를 빌드 | canonical 콘텐츠 hash 동일, wall-clock metadata 는 hash 제외 | NOT_RUN |
| [P1-BT-002](02_Phase1_콘텐츠_자산_빌드파이프라인_상세설계서.md#p1-bt-002) | BT | FUNC-P1-002 | 태그 허용과 금지 동시 지정 | ERROR; 합리화하여 자동 수정하지 않음 | NOT_RUN |
| [P1-FT-002](02_Phase1_콘텐츠_자산_빌드파이프라인_상세설계서.md#p1-ft-002) | FT | FUNC-P1-002 | DB 빌드 중 I/O 실패 | 직전 승인 bundle 유지; 절반짜리 bundle 비활성 | NOT_RUN |
| [P1-CT-002](02_Phase1_콘텐츠_자산_빌드파이프라인_상세설계서.md#p1-ct-002) | CT | FUNC-P1-002 | 두 번 동일 소스를 빌드; 같은요청2 회 | canonical 콘텐츠 hash 동일, wall-clock metadata 는 hash 제외; 동일 commandId/payload 반복은 효과1 회, 같은 ID/다른 payload 는 IdempotencyKeyReuse | NOT_RUN |
| [P1-IT-002](02_Phase1_콘텐츠_자산_빌드파이프라인_상세설계서.md#p1-it-002) | IT | FUNC-P1-002 | 두 번 동일 소스를 빌드; 모듈 adapter 를실제 구현으로교체 | canonical 콘텐츠 hash 동일, wall-clock metadata 는 hash 제외; 앱/헤드리스 entry 가 동일핵심 use case 를호출하고 새세션으로재조회시동일결과 | NOT_RUN |
| [P1-UT-003](02_Phase1_콘텐츠_자산_빌드파이프라인_상세설계서.md#p1-ut-003) | UT | FUNC-P1-003 | NPC-W-03147, BATTLE_TOKEN | 동일 파일의 1:1 crop 과 원래 key 유지 | NOT_RUN |
| [P1-BT-003](02_Phase1_콘텐츠_자산_빌드파이프라인_상세설계서.md#p1-bt-003) | BT | FUNC-P1-003 | 해당 파일 없음 | 여성 generic fallback 표시; NPC identity 불변 | NOT_RUN |
| [P1-FT-003](02_Phase1_콘텐츠_자산_빌드파이프라인_상세설계서.md#p1-ft-003) | FT | FUNC-P1-003 | ../../save.db 를 asset key 로 입력 | InvalidAssetKey; 파일 읽지 않음 | NOT_RUN |
| [P1-CT-003](02_Phase1_콘텐츠_자산_빌드파이프라인_상세설계서.md#p1-ct-003) | CT | FUNC-P1-003 | NPC-W-03147, BATTLE_TOKEN; 같은요청2 회 | 동일 파일의 1:1 crop 과 원래 key 유지; 동일 commandId/payload 반복은 효과1 회, 같은 ID/다른 payload 는 IdempotencyKeyReuse | NOT_RUN |
| [P1-IT-003](02_Phase1_콘텐츠_자산_빌드파이프라인_상세설계서.md#p1-it-003) | IT | FUNC-P1-003 | NPC-W-03147, BATTLE_TOKEN; 모듈 adapter 를실제 구현으로교체 | 동일 파일의 1:1 crop 과 원래 key 유지; 앱/헤드리스 entry 가 동일핵심 use case 를호출하고 새세션으로재조회시동일결과 | NOT_RUN |
| [P1-UT-004](02_Phase1_콘텐츠_자산_빌드파이프라인_상세설계서.md#p1-ut-004) | UT | FUNC-P1-004 | 옛 ID alias OLD-WPN→WPN-0001 | 변환 이력과 이전 ID 보존 | NOT_RUN |
| [P1-BT-004](02_Phase1_콘텐츠_자산_빌드파이프라인_상세설계서.md#p1-bt-004) | BT | FUNC-P1-004 | 선택 이미지 팩 비활성 | 기본 portrait/fallback 으로 정상 표시 | NOT_RUN |
| [P1-FT-004](02_Phase1_콘텐츠_자산_빌드파이프라인_상세설계서.md#p1-ft-004) | FT | FUNC-P1-004 | 필수 스킬 ID 에 alias/legacy 없음 | 로드 차단·원본 세이브 보존 | NOT_RUN |
| [P1-CT-004](02_Phase1_콘텐츠_자산_빌드파이프라인_상세설계서.md#p1-ct-004) | CT | FUNC-P1-004 | 옛 ID alias OLD-WPN→WPN-0001; 같은요청2 회 | 변환 이력과 이전 ID 보존; 동일 commandId/payload 반복은 효과1 회, 같은 ID/다른 payload 는 IdempotencyKeyReuse | NOT_RUN |
| [P1-IT-004](02_Phase1_콘텐츠_자산_빌드파이프라인_상세설계서.md#p1-it-004) | IT | FUNC-P1-004 | 옛 ID alias OLD-WPN→WPN-0001; 모듈 adapter 를실제 구현으로교체 | 변환 이력과 이전 ID 보존; 앱/헤드리스 entry 가 동일핵심 use case 를호출하고 새세션으로재조회시동일결과 | NOT_RUN |
| [P1-RT-001](02_Phase1_콘텐츠_자산_빌드파이프라인_상세설계서.md#p1-rt-001) | RT | PHASE-1 | WPN-0001 물리19 레벨1; 선행 Phase 의승인 fixture 전체 | 원문 ID 와 수치가 content_template 에 일치; 선행의권위 hash/금액/아이템/시간/기존오류동작동일 | NOT_RUN |
| [P1-CN-001](02_Phase1_콘텐츠_자산_빌드파이프라인_상세설계서.md#p1-cn-001) | CN | PHASE-1 | WPN-0001 물리19 레벨1; 요청2 개동시에제출/이전 epoch 응답지연 | mutation 은직렬화·동일 명령효과1 회·오래된 epoch 쓰기0; 순수/도구기능은출력동치및독립임시경로,live 쓰기0 | NOT_RUN |
| [P1-REC-001](02_Phase1_콘텐츠_자산_빌드파이프라인_상세설계서.md#p1-rec-001) | REC | PHASE-1 | 참조되지 않는 MON ID 가 loot 에 있음; 정상요청직전/커밋직전/직후 kill | 원문 규칙상예상실패를유지하면서완전이전또는완전다음세대/산출물만보존·부분 혼합0 | NOT_RUN |
| [P1-PT-001](02_Phase1_콘텐츠_자산_빌드파이프라인_상세설계서.md#p1-pt-001) | PT | PHASE-1 | WPN-0001 물리19 레벨1; seed0..99 를반복하고대표최대 fixture 사용 | 결과와 bounded 종료확인; latency/PSS/DB bytes 실측기록. 성능목표는 P24 표/본 Phase 특화 fixture 에대조하며미측정 PASS 금지 | NOT_RUN |
| [P1-OP-001](02_Phase1_콘텐츠_자산_빌드파이프라인_상세설계서.md#p1-op-001) | OP | PHASE-1 | 옛 ID alias OLD-WPN→WPN-0001; 네트워크차단·앱재실행/도구재실행 | 변환 이력과 이전 ID 보존; 필수 네트워크요청0·게임현실시간 catchup0 | NOT_RUN |
| [P1-ET-001](02_Phase1_콘텐츠_자산_빌드파이프라인_상세설계서.md#p1-et-001) | ET | PHASE-1 | 필수 스킬 ID 에 alias/legacy 없음 | 로드 차단·원본 세이브 보존; 권위 상태오류는안전정지,이미지/파생리포트오류는격리·로그에오류범위명시 | NOT_RUN |
| [P1-IT-005](02_Phase1_콘텐츠_자산_빌드파이프라인_상세설계서.md#p1-it-005) | IT | PHASE-1 | WPN-0001 물리19 레벨1→옛 ID alias OLD-WPN→WPN-0001 | 원문 ID 와 수치가 content_template 에 일치 및 변환 이력과 이전 ID 보존; 선행 port/DTO/version 인계완료 | NOT_RUN |
