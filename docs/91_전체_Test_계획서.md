...
| [P2-BT-001](03_Phase2_월드명령_시간_예약_RNG_상세설계서.md#p2-bt-001) | BT | FUNC-P2-001 | 과거 expectedVersion, 두 번째 world_state 또는 `id!='WORLD'`, 비정규 ResourceIdentity | stale command는 Conflict·변경0. P2 conformance에서 singleton/identity 위반 delta 거절; P3 실제 SQLite/Room에서는 invalid INSERT 자체가 실패 | NOT_RUN |
...
| [P2-IT-001](03_Phase2_월드명령_시간_예약_RNG_상세설계서.md#p2-it-001) | IT | FUNC-P2-001 | 동일 commandId로 금화40 지출 2회, 잔액100; `InMemorySavePort`와 `FaultInjectingSavePort` | 잔액60·receipt 1개·stateVersion 1회 증가, complete-or-previous, failed commit publish 0 | NOT_RUN |
| [P2-UT-002](03_Phase2_월드명령_시간_예약_RNG_상세설계서.md#p2-ut-002) | UT | FUNC-P2-002 | 동일 immutable clock=(분0,잔여0), RNG snapshots, 10초×6와 60초; PCG initState=42/initSeq=54 vector | ClockDelta=(분1,잔여0) 동치, PCG vector `a15c02b7..cbed606e` 동치, changed stream/counter 동치 | NOT_RUN |
| [P2-BT-002](03_Phase2_월드명령_시간_예약_RNG_상세설계서.md#p2-bt-002) | BT | FUNC-P2-002 | 23:59+1분, 12월30일; `worldSeed=0123456789abcdef`, leaf streamKey | 다음 연도1월1일00:00이며 UTF-8 16-byte hex 해석과 다른 raw 8-byte golden을 정확히 선택 | NOT_RUN |
...
| [P2-CT-002](03_Phase2_월드명령_시간_예약_RNG_상세설계서.md#p2-ct-002) | CT | FUNC-P2-002 | combat/<id>/hit raw draw count만 1회와 N회; combat/<id>/crit, loot/<id>, npc/<id>/decision 동일 seed | hit만 변화하고 crit/loot/NPC outputs 및 counters는 불변; 같은 kernel input 반복 결과 동치 | NOT_RUN |
| [P2-IT-002](03_Phase2_월드명령_시간_예약_RNG_상세설계서.md#p2-it-002) | IT | FUNC-P2-002 | 동일 worldSeed/streamKey/clock을 outer AdvanceTime 또는 Combat gameplay command로 JVM·Android에서 실행 | crossed boundary 누락 0, outer receipt 1개, F002 receipt/event 0개, JVM/Android vector 동치. 실제 Room round-trip은 P3 Gate | NOT_RUN |
...
| [P2-UT-004](03_Phase2_월드명령_시간_예약_RNG_상세설계서.md#p2-ut-004) | UT | FUNC-P2-004 | 같은 minute의 forced stop+NPC death+ACTION_START/COMPLETE 4개+economy, health/death→succession과 NPC→economy conditional candidate, `TIME_ADVANCE_GOAL.v1` 5 variant | candidate 누락 0, conditional candidate는 batch 시작에 선언되어 정확히 1회 apply 또는 NoOp, phaseRank/tie-break 순 event order, 동일 stateHash/RNG/counter; UntilEvent 포함 goal payload 동치 | NOT_RUN |
...
| [P2-IT-005](03_Phase2_월드명령_시간_예약_RNG_상세설계서.md#p2-it-005) | IT | FUNC-P2-005 | actual WorldSession과 in-memory SavePort에서 active AdvanceTime pause/cancel/close/새 session restore | PAUSE/CLOSE는 INTERRUPTED와 새 continuation 가능, CANCEL_ADVANCE는 CANCELLED/COMMITTED와 continuation 거절·독립 새 command만 허용하며 F005 자체 receipt/event 없음 | NOT_RUN |
...
| [P2-REC-001](03_Phase2_월드명령_시간_예약_RNG_상세설계서.md#p2-rec-001) | REC | PHASE-2 | segment 계산 중, commit 직전, commit 성공 직후/publish 직전, 다음 segment 진입 직전의 test-only fault cutpoint | double apply/event 누락·중복/RNG double draw/reservation mismatch 0, 마지막 완전 boundary만 보존하고 이미 committed BoundaryKey 재실행 0 | NOT_RUN |
