...
- gameplay 상태 변경은 `WorldSession`의 단일 writer/commit 경로만 사용한다. Phase 3 maintenance/lifecycle의 candidate·slot 변경은 배타 coordinator lease와 84 Command/Event §7의 별도 operation 경계가 소유하며 gameplay receipt/event를 만들지 않는다. `WorldEngine`은 immutable snapshot에서 `ExecutionPlan`만 계산한다.
...
이 다이어그램은 논리 처리 단계다. `SNAPSHOTTED/WRITING/ABORTED`는 transaction 또는 staging의 비영속 단계, `COMMITTED`만 `save_generation.status`의 durable 값이다. `RETAINED/GC_CANDIDATE`는 root 도달성에서 계산하고 `DELETED`는 행 부재이며 별도 DB enum으로 저장하지 않는다.
...
| P3 | `FUNC-P3-003` | 장기진행·예약 복구 | `FOUND → VALIDATED → RESTORED 또는 FALLBACK_OFFERED` | `AUTHORITATIVE` |
