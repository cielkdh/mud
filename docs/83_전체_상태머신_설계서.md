...
| P1 | `FUNC-P1-001` | 정적 카탈로그 스키마와 ID 보존 | `RAW → PARSED → VALIDATED → PUBLISHED` | `BUILD_ARTIFACT` |
| P1 | `FUNC-P1-002` | 콘텐츠 검증·사전 DB 빌드 | `DRAFT → CHECKED → STAGED → ACTIVE` | `BUILD_ARTIFACT` |
| P1 | `FUNC-P1-003` | 로컬 이미지·AssetResolver·크롭 | `RESOLVE → LOCAL/OVERRIDE/FALLBACK` | `BUILD_ARTIFACT` |
...
| P2 | `FUNC-P2-002` | 게임 달력·잔여 밀리초·RNG 스트림 | `(minute,remainder) → canonical clock + stream snapshots` | `AUTHORITATIVE` |
| P2 | `FUNC-P2-003` | 예약·점유·자원 선점 | `PLANNED → RESERVED → RUNNING → COMPLETED/CANCELLED/FAILED` | `AUTHORITATIVE` |
| P2 | `FUNC-P2-004` | 이벤트 경계 시간진행·자동중단 | `IDLE → ADVANCING → INTERRUPTED/COMPLETED/FAILED` | `AUTHORITATIVE` |
...
| P9 | `FUNC-P9-004` | 패배·구조·SAFE_RECOVERY | `DEFEATED → RESCUE_CHECK → RESCUED/SAFE_RECOVERY → HUB` | `AUTHORITATIVE` |
