...
P0 allowlist의 허용 edge는 `:app → :core:simulation` 하나다. 역방향, `:core:simulation`의 Android/Room/Compose/네트워크 의존, 미선언 module/plugin/dependency는 금지 edge로 검사한다.
...
C01~C22의 설계 선택은 모두 승인됐다. 현재 즉시 활성화할 수 있는 범위는 P0의 저장소 인벤토리·버전 lock·최소 compile·모듈/API 경계 검사뿐이며, 이는 C14 미결정 때문이 아니라 코드/빌드 증거가 아직 `NOT_RUN`이기 때문이다. 나머지 Task는 요구 추적용 후보이며, 다음 조건을 모두 채우기 전에는 구현 티켓이나 납기 약속으로 취급하지 않는다.
