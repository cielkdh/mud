...
제작·연구 시간과 재료 선점은 Phase 2 `WorldTimeTraversal(NORMAL_ACTION)`, ACTION_START/ACTION_COMPLETE, claim별 `ResourceClaimPolicy.v1`을 사용한다. 제작은 별도 due scanner나 top-level 소비 정책을 만들지 않는다. 제작/정련별 resumable·progress basis·작업자 교체 허용 여부·stage별 재료 손실과 consequence event codec을 `ActionKindPolicyProfile.v1` fixture로 제공한다.
